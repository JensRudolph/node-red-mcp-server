...
function normalizeMaxStringLength(value, fallback = DEFAULT_MAX_STRING_LENGTH) {
...
export function setValueAtPath(value, path, nextValue) {
...
export function deleteValueAtPath(value, path) {
  const parts = parsePath(path);
  if (parts.length === 0) {
    return;
  }
...
export function shapeValueForResponse(value, options = {}, path = []) {
  const maxStringLength = normalizeMaxStringLength(
    options.maxStringLength,
    options.defaultMaxStringLength ?? DEFAULT_MAX_STRING_LENGTH
  );
  const truncations = [];

  function transform(item, itemPath) {
    if (typeof item === "string") {
      if (options.includeFullValues === true || item.length <= maxStringLength) {
        return item;
      }

      truncations.push({
        field: pathToString(itemPath),
        originalLength: item.length,
        returnedLength: maxStringLength,
      });
      return `${item.slice(0, maxStringLength)}... [truncated ${item.length - maxStringLength} chars]`;
    }

    if (Array.isArray(item)) {
      return item.map((child, index) => transform(child, itemPath.concat(index)));
    }

    if (isObject(item)) {
      const result = {};
      for (const [key, child] of Object.entries(item)) {
        result[key] = transform(child, itemPath.concat(key));
      }
      return result;
    }
...
export function shapeNodeForResponse(node, options = {}) {
  const alwaysFields = options.alwaysFields || ["id", "type", "name", "label", "z", "g"];
  const fields = Array.isArray(options.fields) ? options.fields.filter(Boolean) : [];
  const omitFields = Array.isArray(options.omitFields)
    ? options.omitFields.filter(Boolean)
    : [];

  let selected;
  if (fields.length > 0) {
    selected = {};
    for (const field of [...alwaysFields, ...fields]) {
      const value = getValueAtPath(node, field);
      if (value !== undefined) {
        setValueAtPath(selected, field, cloneJson(value));
      }
    }
  } else {
    selected = cloneJson(node);
  }

  for (const field of omitFields) {
    deleteValueAtPath(selected, field);
  }

  const shaped = shapeValueForResponse(selected, options);
  if (shaped.truncations.length > 0 && isObject(shaped.value)) {
    shaped.value._mcp = {
      ...(shaped.value._mcp || {}),
      truncatedFields: shaped.truncations,
    };
  }
...
export function shapeNodesForResponse(nodes, options = {}) {
  return (Array.isArray(nodes) ? nodes : []).map((node) =>
    shapeNodeForResponse(node, options)
  );
}
...
export function searchNodeFields(nodes, options = {}) {
  ...
  for (const node of nodes) {
    visitValues(node, ({ value, pathString }) => {
      ...
      const shaped = shapeValueForResponse(value, options, [pathString]);
      const truncation = shaped.truncations[0] || null;
      matches.push({
        id: node.id,
        type: node.type || "",
        name: getNodeName(node),
        ...
        originalLength: truncation?.originalLength ?? null,
      });
    });
  }
