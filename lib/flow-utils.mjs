/**
 * Validate and normalize a single-flow object for Node-RED's direct
 * PUT /flow/:id endpoint.
 * @param {string} flowId - Flow/tab id to replace
 * @param {Object} flowConfig - Single-flow object from /flow/:id format
 * @returns {Object} Single-flow update payload
 */
export function buildDirectFlowUpdatePayload(flowId, flowConfig) {
  if (flowId === "global") {
    throw new Error("Direct update-flow does not support the global flow");
  }
  ...
  const normalizedNodes = nodes.map((node) => {
    if (!node || typeof node !== "object" || Array.isArray(node)) {
      throw new Error("Flow nodes must be objects");
    }

    if (node.type === "tab") {
      throw new Error("Flow nodes must not contain tab nodes");
    }
    ...
    return node.z === undefined ? { ...node, z: flowId } : node;
  });

  for (const config of configs) {
    if (!config || typeof config !== "object" || Array.isArray(config)) {
      throw new Error("Flow configs must be objects");
    }

    if (config.type === "tab") {
      throw new Error("Flow configs must not contain tab nodes");
    }

    if (config.z !== undefined && config.z !== flowId) {
      throw new Error(
        `Config node '${config.id || "(unknown)"}' belongs to flow '${config.z}', not '${flowId}'`
      );
    }
  }
...
export function replaceFlowInCompleteFlows(flows, flowId, flowConfig) {
  ...
  const directPayload = buildDirectFlowUpdatePayload(flowId, flowConfig);
  ...
  const replacementMembers = [...nodes, ...configs];
