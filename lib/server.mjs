...
const defaultConfig = {
  ...
  readOnly: false,
  minimal: false,
  allowFullFlowWrites: false,
  mutationConfirmationThreshold: 50,
...
function normalizeConfig(userConfig) {
  ...
  return {
    ...
    minimal: firstDefined(
      userConfig.minimal,
      process.env.MCP_MINIMAL !== undefined
        ? parseBoolean(process.env.MCP_MINIMAL)
        : undefined,
      defaultConfig.minimal
    ),
    allowFullFlowWrites: firstDefined(
      userConfig.allowFullFlowWrites,
      process.env.MCP_ALLOW_FULL_FLOW_WRITES !== undefined
        ? parseBoolean(process.env.MCP_ALLOW_FULL_FLOW_WRITES)
        : undefined,
      defaultConfig.allowFullFlowWrites
    ),
    mutationConfirmationThreshold: firstDefined(
      userConfig.mutationConfirmationThreshold !== undefined
        ? parsePositiveInteger(
            userConfig.mutationConfirmationThreshold,
            "mutationConfirmationThreshold"
          )
        : undefined,
      parsePositiveInteger(
      ...
      defaultConfig.mutationConfirmationThreshold
    ),
    maxResponseItems: firstDefined(
      userConfig.maxResponseItems !== undefined
        ? parsePositiveInteger(userConfig.maxResponseItems, "maxResponseItems")
        : undefined,
      parsePositiveInteger(process.env.MCP_MAX_RESPONSE_ITEMS, "MCP_MAX_RESPONSE_ITEMS"),
      defaultConfig.maxResponseItems
    ),
    maxStringLength: firstDefined(
      userConfig.maxStringLength !== undefined
        ? parsePositiveInteger(userConfig.maxStringLength, "maxStringLength")
        : undefined,
      parsePositiveInteger(process.env.MCP_MAX_STRING_LENGTH, "MCP_MAX_STRING_LENGTH"),
      defaultConfig.maxStringLength
    ),
    backup,
  };
}

function createFilteringServer(server, config) {
  if (!config.minimal) return server;

  const allowed = new Set(["backup-flows", "restore-backup-flows"]);
  const original = server.tool.bind(server);

  server.tool = (name, ...args) => {
    if (allowed.has(name)) return original(name, ...args);
  };
...
export function createServer(userConfig = {}) {
  ...
  const filtered = createFilteringServer(server, config);

  // Register all tools — filtering is handled by the wrapper above
  registerFlowTools(filtered, config);
  registerNodeTools(filtered, config);
  registerSettingsTools(filtered, config);
  registerUtilityTools(filtered, config);
  registerBackupTools(filtered, config);
...
export { defaultConfig, normalizeConfig, createFilteringServer };
