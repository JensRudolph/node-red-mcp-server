...
export default function registerFlowTools(server, config) {
  ...
  server.tool(
    ...
    async (args) => runTool("Dry-run create flow", async () => {
      ...
      return jsonResponse({
        ...
        conflicts,
        entities: auditEntitiesInFlow(flowObj),
        internalLinks: summarizeInternalLinks(flowObj),
        counts: {
  ...
  server.tool(
    ...
    async (args) => runTool("Entity audit", async () => {
      ...
      return jsonResponse(auditEntitiesInFlow(flowObj));
    })
  );
  ...
  server.tool(
    ...
    async ({ sourceFlowId, targetFlowId, idMap, expectedReplacements }) =>
      runTool("Diff flow against source", async () => {
        ...
        return jsonResponse(
          createFlowDiff(sourceArray, targetArray, {
            sourceFlowId,
            targetFlowId,
      ...
      async (args) => runTool("Clone flow", async () => {
        ...
        if (normalizeDryRun(args.dryRun)) {
          return jsonResponse({
            ...
            wouldWrite: false,
            ...clone,
            entities: auditEntitiesInFlow(clone.flow),
            internalLinks: summarizeInternalLinks(clone.flow),
          });
        }
      ...
      async (args) => runTool("Clear entities in flow", async () => {
        ...
        if (normalizeDryRun(args.dryRun)) {
          return jsonResponse({
            ...
            changes: result.changes,
            validation,
            entities: auditEntitiesInFlow(result.flow),
          });
        }
