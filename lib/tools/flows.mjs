...
function shapeFlowConfigForResponse(flow, args, config) {
  const { nodes = [], configs = [], subflows = [], ...tabFields } = flow;
  const options = responseShapeOptions(args, config);
  const tabOptions = {
    ...options,
    fields: undefined,
  ...
  return {
    ...shapeNodeForResponse(tabFields, tabOptions),
    nodes: shapeNodesForResponse(nodes, options),
    configs: shapeNodesForResponse(configs, options),
    ...(Array.isArray(subflows)
      ? { subflows: shapeNodesForResponse(subflows, options) }
      : {}),
  };
}
...
function getIdsOutsideFlow(flows, flowId) {
  return new Set(
    (flows || [])
      .filter((node) => node?.id && node.id !== flowId && node.z !== flowId)
      .map((node) => node.id)
  );
}

function getKnownExistingIds(flows) {
  return getAllIds(flows || []);
}

function getKnownExternalLinkIdsForPayload(existingFlows, payload) {
  const flowId = !Array.isArray(payload) ? payload?.id || null : null;
  const replacesExistingFlow =
    flowId &&
    (existingFlows || []).some(
      (node) => node?.id === flowId && node.type === "tab"
    );

  return replacesExistingFlow
    ? getIdsOutsideFlow(existingFlows, flowId)
    : getKnownExistingIds(existingFlows);
}

function formatUsageForResponse(usage, args, config) {
  return {
    subflowId: usage.subflowId,
    instanceCount: usage.instanceCount,
    byFlow: usage.byFlow,
    instances: limitedList(usage.instances, {
      include: args.includeInstances ?? true,
      limit: getResponseLimit(args, config),
    }),
  };
}

function summarizeSubflowsForResponse(flows, args, config) {
  const subflows = (flows || [])
    .filter((node) => node?.type === "subflow")
    .map((subflow) => {
      const members = (flows || []).filter((node) => node?.z === subflow.id);
      const usage = summarizeSubflowUsage(flows, subflow.id);
      return {
        id: subflow.id,
        name: subflow.name || "",
  ...
  return limitedList(subflows, {
    include: true,
    limit: getResponseLimit(args, config),
  });
}

function summarizeSubflowPayload(payload) {
  const normalized = normalizeSubflowConfig(payload);
  return {
    id: normalized.subflow.id,
    name: normalized.subflow.name || "",
    nodeCount: normalized.nodes.length,
    configCount: normalized.configs.length,
    objectCount: subflowConfigToFlatArray(normalized).length,
  };
}
...
export default function registerFlowTools(server, config) {
  ...
  server.tool(
    ...
    async (args) => runTool("Get flows", async () => {
      const flows = await callNodeRed("get", "/flows", null, config);
      const selection = hasSelectiveGetFlowsArgs(args)
        ? selectFlows(flows, args)
        : flows;
      return jsonResponse(shapeFlowSelectionForResponse(selection, args, config));
    })
  );
  ...
  server.tool(
    ...
    async (args) => runTool("Get flow", async () => {
      const flow = await callNodeRed(
        "get",
        `/flow/${encodeURIComponent(args.id)}`,
        null,
        config
      );
      return jsonResponse(shapeFlowConfigForResponse(flow, args, config));
    })
  );
  ...
  server.tool(
    "list-subflows",
    "Lists subflow definitions with internal node counts and instance counts.",
    {
      includeUsage: z
        .boolean()
        .optional()
        .describe("Include per-flow usage details for each subflow"),
      limitItems: z.number().int().nonnegative().optional().describe("Limit returned subflows and usage items"),
    },
    async (args) => runTool("List subflows", async () => {
      const flows = await callNodeRed("get", "/flows", null, config);
      return jsonResponse({
        subflows: summarizeSubflowsForResponse(flows, args, config),
      });
    })
  );
  ...
  server.tool(
    "subflow-usage",
    "Finds all instances of one subflow across flow tabs.",
    {
      id: z.string().describe("Subflow ID"),
      includeInstances: z
        .boolean()
        .optional()
        .describe("Include individual subflow instance nodes"),
      limitItems: z.number().int().nonnegative().optional().describe("Limit returned instance items"),
    },
    async (args) => runTool("Subflow usage", async () => {
      const flows = await callNodeRed("get", "/flows", null, config);
      getSubflowDefinition(flows, args.id);
      const usage = summarizeSubflowUsage(flows, args.id);
      return jsonResponse(formatUsageForResponse(usage, args, config));
    })
  );

  server.tool(
    "validate-subflow-payload",
    "Validates a subflow payload, including internal wires, links, groups and subflow in/out/status references.",
    {
      id: z.string().optional().describe("Expected subflow ID"),
      subflow: flowConfigSchema.optional().describe("Subflow payload, either {subflow,nodes,configs} or a nested subflow object"),
      subflowJson: z.string().optional().describe("Deprecated fallback: subflow payload as JSON"),
    ...
    async (args) => runTool("Validate subflow payload", async () => {
      const payload = readStructuredOrJsonArgument(args, "subflow", "subflowJson");
      const catalog = await loadConfigRefCatalog();
      const existingFlows = await callNodeRed("get", "/flows", null, config);
      const normalized = normalizeSubflowConfig(payload, { id: args.id });
      return jsonResponse(
        validateSubflowPayload(normalized, {
          ...
          knownExternalLinkIds: getIdsOutsideFlow(
            existingFlows,
  ...
  server.tool(
    "dry-run-update-subflow",
    "Previews replacing one subflow through PUT /flow/global without writing.",
    {
      id: z.string().describe("Subflow ID to replace"),
      subflow: flowConfigSchema.optional().describe("Replacement subflow payload, either {subflow,nodes,configs} or a nested subflow object"),
      subflowJson: z.string().optional().describe("Deprecated fallback: replacement subflow payload as JSON"),
      allowEmptyEntities: z.boolean().optional(),
      strictEntities: z.boolean().optional(),
      ...
      includeInstances: z.boolean().optional().describe("Include subflow instance details"),
      includePayload: z.boolean().optional().describe("Include normalized replacement payload"),
      limitItems: z.number().int().nonnegative().optional().describe("Limit returned detail items"),
    },
    async (args) => runTool("Dry-run update subflow", async () => {
      const payload = readStructuredOrJsonArgument(args, "subflow", "subflowJson");
      const normalized = normalizeSubflowConfig(payload, { id: args.id });
      const flows = await callNodeRed("get", "/flows", null, config);
      const catalog = await loadConfigRefCatalog();
      const current = getSubflowDefinition(flows, normalized.subflow.id);
      const validation = validateSubflowPayload(normalized, {
        allowEmptyEntities: args.allowEmptyEntities ?? true,
        strictEntities: args.strictEntities ?? false,
        allowExternalConfigRefs: true,
        knownExternalLinkIds: getIdsOutsideFlow(flows, normalized.subflow.id),
        configRefCatalog: catalog,
      });
      const previewDiff = createFlowDiff(
        subflowConfigToFlatArray(current),
        subflowConfigToFlatArray(normalized),
        {
          operation: "update-subflow-preview",
          subflowId: normalized.subflow.id,
        }
      );
      const entityAudit = auditEntitiesInFlow({
        nodes: normalized.nodes,
        configs: normalized.configs,
      });
      const usage = summarizeSubflowUsage(flows, normalized.subflow.id);

      return jsonResponse({
        dryRun: true,
        wouldWrite: false,
        summary: summarizeSubflowPayload(normalized),
        validation,
        diff: { summary: previewDiff.summary },
        usage: formatUsageForResponse(usage, args, config),
        entities: {
        ...
        internalLinks: limitedList(summarizeInternalLinks({
          nodes: normalized.nodes,
          configs: normalized.configs,
        }), {
          include: args.includeInternalLinks ?? false,
          limit: getResponseLimit(args, config),
        }),
        ...(args.includePayload ? { subflow: normalized } : {}),
      });
    })
  );

  server.tool(
    "dry-run-create-subflow",
    "Previews creating a new subflow through PUT /flow/global without writing.",
    {
      subflow: flowConfigSchema.optional().describe("New subflow payload, either {subflow,nodes,configs} or a nested subflow object"),
      subflowJson: z.string().optional().describe("Deprecated fallback: new subflow payload as JSON"),
      allowEmptyEntities: z.boolean().optional(),
      strictEntities: z.boolean().optional(),
      includeEntities: z.boolean().optional().describe("Include entity occurrence details"),
      includeInternalLinks: z.boolean().optional().describe("Include internal link details"),
      includePayload: z.boolean().optional().describe("Include normalized subflow payload"),
      limitItems: z.number().int().nonnegative().optional().describe("Limit returned detail items"),
    },
    async (args) => runTool("Dry-run create subflow", async () => {
      const payload = readStructuredOrJsonArgument(args, "subflow", "subflowJson");
      const normalized = normalizeSubflowConfig(payload);
      const flows = await callNodeRed("get", "/flows", null, config);
      const catalog = await loadConfigRefCatalog();
      const existingIds = getAllIds(flows);
      const conflicts = subflowConfigToFlatArray(normalized)
      ...
      const validation = validateSubflowPayload(normalized, {
        ...
        strictEntities: args.strictEntities ?? false,
        allowExternalConfigRefs: true,
        knownExternalLinkIds: getKnownExistingIds(flows),
        configRefCatalog: catalog,
      });
      const entityAudit = auditEntitiesInFlow({
        nodes: normalized.nodes,
    ...
    async (args) => runTool("Validate flow payload", async () => {
      ...
      return jsonResponse(
        validateFlowPayload(payload, {
          ...
          strictEntities: args.strictEntities ?? false,
          allowExternalConfigRefs: args.allowExternalConfigRefs,
          knownExternalLinkIds: getKnownExternalLinkIdsForPayload(
            existingFlows,
            payload
          ),
          configRefCatalog: catalog,
    ...
    async (args) => runTool("Dry-run create flow", async () => {
      ...
      return jsonResponse({
        ...
        validation: validateFlowPayload(flowObj, {
          allowEmptyEntities: args.allowEmptyEntities ?? true,
          allowExternalConfigRefs: true,
          knownExternalLinkIds: getKnownExistingIds(existingFlows),
          configRefCatalog: catalog,
        }),
        conflicts,
        entities: {
    ...
    server.tool(
      ...
      async (args) => runTool("Update flow", async () => {
          ...
          const validation = validateFlowPayload(payload, {
            allowEmptyEntities: true,
            allowExternalConfigRefs: true,
            knownExternalLinkIds: getIdsOutsideFlow(currentFlows, id),
            configRefCatalog: catalog,
          });
          if (!validation.valid) {
            return jsonResponse({
  ...
  if (!config.readOnly) {
    server.tool(
      "update-subflow",
      "Replaces one subflow through Node-RED's scoped PUT /flow/global endpoint. Prefer dry-run-update-subflow first.",
      {
        id: z.string().describe("Subflow ID to replace"),
        subflow: flowConfigSchema
          .optional()
          .describe("Replacement subflow payload, either {subflow,nodes,configs} or a nested subflow object"),
        subflowJson: z
          .string()
          .optional()
          .describe("Deprecated fallback: replacement subflow payload as JSON"),
        allowEmptyEntities: z.boolean().optional(),
        strictEntities: z.boolean().optional(),
        confirmToken: confirmationArgSchema,
      },
      async (args) => runTool("Update subflow", async () => {
        const payload = readStructuredOrJsonArgument(args, "subflow", "subflowJson");
        const normalized = normalizeSubflowConfig(payload, { id: args.id });
        const flows = await callNodeRed("get", "/flows", null, config);
        const catalog = await loadConfigRefCatalog();
        ...
        const confirmation = checkLargeMutation(args, config, {
          operation: "update-subflow",
          scope: normalized.subflow.id,
          affectedCount: countDiffAffected(previewDiff),
          deletedCount: previewDiff.summary.removed || 0,
        });
        if (confirmation.required && !confirmation.confirmed) {
        ...
        const audit = await runMutationWithBackup(
          config,
          `Before update-subflow ${normalized.subflow.id}`,
          async () => {
            const globalFlow = await callNodeRed(
              "get",
              "/flow/global",
              null,
              config
            );
            const nextGlobal = replaceSubflowInGlobalFlow(
              globalFlow,
              normalized
            );
            return callNodeRed("put", "/flow/global", nextGlobal, config);
          }
        );
        ...
        return jsonResponse({
          ...
          confirmation,
          diff: { summary: previewDiff.summary },
          usage: formatUsageForResponse(
            summarizeSubflowUsage(flows, normalized.subflow.id),
  ...
  if (!config.readOnly) {
    server.tool(
      "create-subflow",
      "Creates a new subflow through Node-RED's scoped PUT /flow/global endpoint. Prefer dry-run-create-subflow first.",
      {
        subflow: flowConfigSchema
          .optional()
          .describe("New subflow payload, either {subflow,nodes,configs} or a nested subflow object"),
        subflowJson: z
          .string()
          .optional()
          .describe("Deprecated fallback: new subflow payload as JSON"),
        allowEmptyEntities: z.boolean().optional(),
        strictEntities: z.boolean().optional(),
        confirmToken: confirmationArgSchema,
      },
      async (args) => runTool("Create subflow", async () => {
        const payload = readStructuredOrJsonArgument(args, "subflow", "subflowJson");
        const normalized = normalizeSubflowConfig(payload);
        const flows = await callNodeRed("get", "/flows", null, config);
        const catalog = await loadConfigRefCatalog();
        ...
        if (conflicts.length > 0 || !validation.valid) {
          return jsonResponse({
            created: false,
            validation,
            conflicts,
        ...
        const affectedCount = subflowConfigToFlatArray(normalized).length;
        const confirmation = checkLargeMutation(args, config, {
          operation: "create-subflow",
          scope: normalized.subflow.name || normalized.subflow.id,
          affectedCount,
        });
        if (confirmation.required && !confirmation.confirmed) {
          return mutationBlockedResponse(confirmation, {
        ...
        const audit = await runMutationWithBackup(
          config,
          `Before create-subflow ${normalized.subflow.id}`,
          async () => {
            const globalFlow = await callNodeRed(
              "get",
              "/flow/global",
            ...
            const nextGlobal = appendSubflowToGlobalFlow(
              globalFlow,
              normalized
            );
            return callNodeRed("put", "/flow/global", nextGlobal, config);
  ...
  if (!config.readOnly) {
    server.tool(
      "clone-subflow",
      "Clones a subflow with deterministic ID remapping, validation, dry-run support, and scoped PUT /flow/global write.",
      {
        sourceId: z.string().describe("Source subflow ID"),
        newName: z.string().describe("Name for the cloned subflow"),
        newId: z.string().optional().describe("Optional deterministic ID for the cloned subflow"),
        ...
        dryRun: z
          .boolean()
          .optional()
          .describe("Default true. Set false to actually create the subflow."),
        confirmToken: confirmationArgSchema,
        includePayload: z.boolean().optional().describe("Include the full cloned subflow payload in the response"),
      ...
      async (args) => runTool("Clone subflow", async () => {
        const flows = await callNodeRed("get", "/flows", null, config);
        const source = getSubflowDefinition(flows, args.sourceId);
        const clone = cloneSubflowConfig(source, {
          existingIds: getAllIds(flows),
          knownExternalLinkIds: getKnownExistingIds(flows),
          newId: args.newId,
        ...
        const catalog = await loadConfigRefCatalog();
        clone.validation = validateSubflowPayload(clone.subflow, {
          allowEmptyEntities: args.allowEmptyEntities ?? true,
          allowExternalConfigRefs: true,
          knownExternalLinkIds: getKnownExistingIds(flows),
          configRefCatalog: catalog,
        ...
        const entityAudit = auditEntitiesInFlow({
          nodes: clone.subflow.nodes,
          configs: clone.subflow.configs,
        });
        ...
        if (normalizeDryRun(args.dryRun)) {
          return jsonResponse({
            ...
            ...(args.includePayload ? { subflow: clone.subflow } : {}),
          });
        }
        ...
        const affectedCount = subflowConfigToFlatArray(clone.subflow).length;
        const confirmation = checkLargeMutation(args, config, {
          operation: "clone-subflow",
          scope: `${args.sourceId}:${args.newName}`,
          affectedCount,
        });
        if (confirmation.required && !confirmation.confirmed) {
          return mutationBlockedResponse(confirmation, {
        ...
        const audit = await runMutationWithBackup(
          config,
          `Before clone-subflow ${args.sourceId}`,
          async () => {
            const globalFlow = await callNodeRed(
              "get",
              "/flow/global",
    ...
    server.tool(
      ...
      async (args) => runTool("Update flow full", async () => {
          ...
          const validation = validateFlowPayload(payload, {
            allowEmptyEntities: true,
            allowExternalConfigRefs: true,
            knownExternalLinkIds: getIdsOutsideFlow(current.flows, id),
            configRefCatalog: catalog,
          });
          if (!validation.valid) {
            return jsonResponse({
