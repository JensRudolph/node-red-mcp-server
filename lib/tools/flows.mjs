...
export default function registerFlowTools(server, config) {
  ...
  server.tool(
    "update-flow",
    "Safely updates the configuration of a specific flow by its id using GET /flows followed by POST /flows with optimistic locking. Args: id (e.g.'396c237c693dc') flowJson (e.g.'{id: '91ad456e52b8', label: 'Sheet 1', nodes: [], configs: []}')",
    {
      id: z.string().describe("Flow ID"),
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Flow ${id} updated safely using optimistic locking.${revText}`,
            },
          ],
        };
      } catch (error) {
