  ...
  if (!config.readOnly) {
    ...
    server.tool(
      ...
      async (args) => runTool("Update flow", async () => {
          ...
          await createMutationBackup(config, `Before direct update-flow ${id}`);
          const result = await callNodeRed(
            "put",
            `/flow/${encodeURIComponent(id)}`,
            payload,
            config
  ...
  if (!config.readOnly) {
    ...
    server.tool(
      ...
      async (args) => runTool("Update flow full", async () => {
          ...
          const flowObj = readStructuredOrJsonArgument(args, "flow", "flowJson");
          const payload = buildDirectFlowUpdatePayload(id, flowObj);
          await createMutationBackup(config, `Before update-flow-full ${id}`);
          const current = await getFlowsWithRevision(config);
          ...
          return textResponse(
            `Flow ${id} updated through complete /flows write using optimistic locking.${revText}`
          );
      })
    );
  }
