  ...
  if (!config.readOnly) {
    ...
    server.tool(
      ...
      async ({ id }) => runTool("Trigger inject node", async () => {
        ...
        return textResponse(`Inject node ${id} triggered`);
      })
    );
  }
  ...
  if (!config.readOnly) {
    ...
    server.tool(
      ...
      async ({ module }) => runTool("Install node module", async () => {
        await createMutationBackup(config, `Before install-node-module ${module}`);
  ...
  if (!config.readOnly) {
    ...
    server.tool(
      ...
      async ({ module, enabled }) => runTool("Toggle node module", async () => {
        assertSafePathPart(module, "module");
        await createMutationBackup(config, `Before toggle-node-module ${module}`);
        await callNodeRed("put", "/nodes/" + module, { enabled }, config);
        return textResponse(`Module ${module} ${enabled ? "enabled" : "disabled"}`);
      })
    );
  }
  if (!config.readOnly) {
    ...
    server.tool(
      ...
      async ({ module, set, enabled }) => runTool("Toggle node module set", async () => {
        ...
        return textResponse(
          `Module ${module} set ${set} ${enabled ? "enabled" : "disabled"}`
        );
      })
    );
  }
